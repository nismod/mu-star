...
from globdata.parameters import model_alias, job_partition, region_lookup, set_model_index
...
# importing from econmy at the end to overwrtie some defaults from globdata
...
rule econ_model_setup:
    ...
    output:
        model_dir=directory(f"{OUTPUT_ROOT}/{model_name}"),
    params:
        output_root=OUTPUT_ROOT,
        catalogue_root=CATALOGUE_ROOT
    log:
        f"logs/{model_name}_setup.log"
    run:
        ...
        CATALOGUE = load_catalogue(root=params.catalogue_root)
        ...
        model_dir = f"{OUTPUT_ROOT}/{model_name}"
        ...
        mirror_model_dir = f"{model_dir}" # f"data/processed/Socio-Economic/BHM/models"
        ...
        cpus_per_task : int = model_params.n_cores
        ...
        if not MIRROR:
            ...
            with open(yaml_params, "w") as f:
                yaml.safe_dump({**model_params.__dict__, **constraints.__dict__}, f, sort_keys=False)

            job_index.to_csv(job_index_path,index=False)

        elif MIRROR:
            os.makedirs(mirror_model_dir, exist_ok=True)
